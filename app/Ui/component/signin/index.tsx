...
import useAuthForm from '@/app/hooks/useSignin';
  ...
  return (
    <div>
      <FormCard>
        ...
        <ErrorMessage id="login" message={errors?.email?.message} />
            <Label>
              <input
                type="text"
          ...
          <div>
              <button className='text-black justify-center flex w-full border border-black outline-none p-2 rounded-md' disabled={formState.isSubmitting}> {formState.isSubmitting ? <Loader/> : 'LOGIN'}</button>
          </div>
        </form>
      </FormCard>
      <StyledFooter>
